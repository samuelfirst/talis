...
class TestTwitchChat(object):
    ...
    @patch('socket.socket')
    def test_connect(
        self, mocked_socket, mocked_logged_in,
        mocked_fcntl, mocked_join_channel
    ):
        mocked_logged_in.return_value = True
        mocked_socket.return_value \
            .recv.return_value \
            .decode.return_value = "data packet"

        twitch_chat = TwitchChat(
            "username", "oauth",
            "channel", "chat_queue", "command_queue",
            "admin_command_queue", "stop_event"
        )
        ...
        mocked_socket.return_value \
            .connect.assert_called_with(("irc.twitch.tv", 6667))
